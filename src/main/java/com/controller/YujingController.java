    ...
    */
    ...
    public R save(@RequestBody YujingEntity yujing, HttpServletRequest request){
        ...
        Wrapper<YujingEntity> queryWrapper = new EntityWrapper<YujingEntity>()
            .eq("yujing_name", yujing.getYujingName())
...
//                            yujingEntity.setYujingName(data.get(0));                    //预警名称 要改的
//                            yujingEntity.setYujingPhoto("");//详情和图片
//                            yujingEntity.setYujingTypes(Integer.valueOf(data.get(0)));   //预警类型 要改的
//                            yujingEntity.setInsertTime(date);//时间
//                            yujingEntity.setYujingContent("");//详情和图片
//                            yujingEntity.setCreateTime(date);//时间
    ...
    */
    @RequestMapping("/add")
    public R add(@RequestBody YujingEntity yujing, HttpServletRequest request){
        logger.debug("add方法:,,Controller:{},,yujing:{}",this.getClass().getName(),yujing.toString());
        Wrapper<YujingEntity> queryWrapper = new EntityWrapper<YujingEntity>()
            .eq("yujing_name", yujing.getYujingName())
            .eq("yujing_types", yujing.getYujingTypes())
//            .notIn("yujing_types", new Integer[]{102})
            ;
        logger.info("sql语句:"+queryWrapper.getSqlSegment());
        YujingEntity yujingEntity = yujingService.selectOne(queryWrapper);
        if(yujingEntity==null){
